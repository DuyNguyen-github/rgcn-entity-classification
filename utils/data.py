...
# Suppress RDFlib warnings about invalid URIs in string literals
...
def st(node):
    """
    Maps an rdflib node to a unique string. We use str(node) for URIs (so they can be matched to the classes),
    we use str() for Literals (to avoid datatype serialization), and we use .n3() for BNodes to maintain uniqueness.
    ...
    if type(node) == URIRef:
        ...
        # For literals, use str() to get just the value without datatype serialization
        return str(node)
    else:
        # For BNodes and other types, use .n3() to maintain uniqueness
        return node.n3()
...
def load_node_classification_data(name, use_test_set=False, limit=None, enable_cache=True, val_prop=0.4, prune=False):
    ...
    if name.lower() == 'aifb':
        ...
        file = locate_file('/data/mutag/mutag_stripped.nt')
